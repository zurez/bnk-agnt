...
<body>
    <div class="container">
        ...
        <footer>
            Generated by DeepEval Model Comparison Tool • Interactive Charts powered by Chart.js
        </footer>
    </div>
...
def generate_comparison_report(result_files: List[Path]) -> tuple[str, str, str]:
    ...
    for result_file in result_files:
        ...
        # Calculate metrics from test_cases instead of parsing results string
        metrics = {}
        passed = 0
        failed = 0
        
        if test_cases:
            # Calculate pass/fail from test cases
            for tc in test_cases:
                tc_metrics = tc.get("metrics", [])
                if tc_metrics and all(m.get("success", False) for m in tc_metrics):
                    passed += 1
                else:
                    failed += 1
            
            # Calculate metric pass rates
            metric_totals = {}
            for tc in test_cases:
                for metric in tc.get("metrics", []):
                    metric_name = metric.get("name", "Unknown")
                    if metric_name not in metric_totals:
                        metric_totals[metric_name] = {"passed": 0, "total": 0}
                    metric_totals[metric_name]["total"] += 1
                    if metric.get("success", False):
                        metric_totals[metric_name]["passed"] += 1
            
            # Convert to pass rates
            for metric_name, totals in metric_totals.items():
                if totals["total"] > 0:
                    metrics[metric_name] = (totals["passed"] / totals["total"]) * 100
        else:
            # Fallback to parsing results string if no test_cases
        ...
        overall_pass_rate = (passed / num_cases * 100) if num_cases > 0 else 0.0
    ...
    for i, model in enumerate(models_data):
        ...
        summary_cards += f"""
            <div class="model-summary-card" onclick="showDetails('{model['name']}')">
                ...
                <div class="click-hint">👆 Click to view detailed results</div>
            </div>
        """
    ...
    charts_script = generate_charts_script(models_data)
...
def generate_charts_script(models_data: List[Dict]) -> str:
...
def main():
    ...
    print(f"📄 Comparing {len(result_files)} model(s):")
